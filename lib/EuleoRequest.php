<?php
/**
 * Euleo-CMS API - class EuleoRequest
 *
 * PHP Version 5.3
 *
...
class EuleoRequest
{
     ...
     * @throws Exception
     */
    public function __construct ($customer = '', $usercode = '')
    {
        ...
        if (!empty($customer) && !empty($usercode)) {
            ...
            $responseXml = $this->client->__soapCall('connect', 
                array(
                    'xml' => $requestXml
                ));
    ...
    public function setLanguageList ($languageList)
    {
        ...
        $responseXml = $this->client->__soapCall('setLanguageList', 
            array(
                'xml' => $requestXml
            ));
        $response = $this->_parseXml($responseXml);
     ...
     * 
     * @param string $translationIdList
     *
     * @return array
     */
    public function getRows ($translationIdList = false)
    {
        $request = array();
        $request['handle'] = $this->handle;
    ...
    public function sendRows ($rows)
    {
        ...
        $responseXml = $this->client->__soapCall('sendRows', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function confirmDelivery (array $translationids)
    {
        ...
        $responseXml = $this->client->__soapCall('confirmDelivery', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function getCart ()
    {
        ...
        $responseXml = $this->client->__soapCall('getCart', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function clearCart ()
    {
        ...
        $responseXml = $this->client->__soapCall('clearCart', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function addLanguage ($code, $language)
    {
        ...
        $responseXml = $this->client->__soapCall('addLanguage', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function removeLanguage ($code, $language)
    {
        ...
        $responseXml = $this->client->__soapCall('removeLanguage', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function identifyLanguages ($texts)
    {
        ...
        $responseXml = $this->client->__soapCall('identifyLanguages', 
            array(
                'xml' => $requestXml
            ));
    ...
    public function setCallbackUrl ($url)
    {
        ...
        $responseXml = $this->client->__soapCall('setCallbackUrl', 
                array(
                        'xml' => $requestXml
                ));
    ...
    protected function _createRequest ($data, $action)
    {
        ...
        $xml[] = '<request action="' . htmlspecialchars($action) . '">';
    ...
    protected function _rowToXml_sub ($row)
    {
        ...
        $lines[] = '<row id="' .
                 htmlspecialchars($row['code'], ENT_COMPAT, 'UTF-8') .
                 '" label="' .
                 htmlspecialchars($row['label'], ENT_COMPAT, 'UTF-8') .
                 '" title="' .
                 htmlspecialchars($row['title'], ENT_COMPAT, 'UTF-8') .
                 ($row['url'] ? '" url="' .
                 htmlspecialchars($row['url'], ENT_COMPAT, 'UTF-8') : '') . '">';
        ...
        if ($row['fields']) {
            ...
            foreach ($row['fields'] as $fieldname => $field) {
                ...
                $lines[] = '<field name="' . htmlspecialchars($fieldname) . '" label="' . htmlspecialchars($label) .
